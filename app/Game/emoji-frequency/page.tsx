...
useEffect(() => {
  ...
  console.log("New sequence:", newSeq); // 👈 helpful for testing
  setSequence(newSeq);
  setIsShowing(true);
  setInput([]);
...
<section className="mx-auto max-w-3xl rounded-2xl bg-white/85 backdrop-blur p-6 shadow-xl">
      ...
      <p className="text-lg text-[#00FFB3]">Level: {level} | Score: {score}</p>
      ...
      {popup && (
        <div className="text-white text-2xl font-bold mt-4 animate-bounce">
          {popup}
        </div>
      )}
      </section>
